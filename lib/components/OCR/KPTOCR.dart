...
class KtpOCR extends StatefulWidget {
  ...
  @override
  State<KtpOCR> createState() => _OcrHomepageState();
}
...
class _OcrHomepageState extends State<KtpOCR> {
  ...
  Future<List<Ktpocr>> KtpOcrApi(File _KtpImage) async {
    ...
    final Url = 'https://5236635838005115.ap-southeast-5.fc.aliyuncs.com/2016-08-15/proxy/ocr/ktp/';
    ...
    if (response.statusCode == 200) {
      ...
      if (status == 'FAILED') {
        ...
        data.add(Ktpocr(date: date, message: message, read: reads, status: status));
      }
    } else {
      setState(() {
        isLoading = false;
